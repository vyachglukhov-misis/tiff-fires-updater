...
export const divideGeojsonOnNSectors = (
  ...
  for (const cell of gridFeatureCollection.features) {
    const normalizedCell = turf.multiPolygon([
      cell.geometry.coordinates as any,
    ]);
    const result = turf.intersect(
      turf.featureCollection([geojson, normalizedCell])
    );
    if (result) intersectedObjects.push(result);
  }

  const intersectedFeatureCollection =
    turf.featureCollection(intersectedObjects);

  return intersectedFeatureCollection;
};
